...
module.exports.create = function create(code, name, credits) {
    const sql = 'CALL create_module($1,$2,$3)';
    return query (sql,[code,name,credits])
    .then(function (result) {
        console.log('Module created successfully')
    })
    .catch(function (error) {
        if(error.code === SQL_ERROR_CODE.UNIQUE_VIOLATION) {
            throw new UNIQUE_VIOLATION_ERROR(`Module ${code} already  exists! Cannot Create Duplicates`)
        }
    })
};
